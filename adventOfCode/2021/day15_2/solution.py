...
def main():
    with open(f"{os.path.dirname(__file__)}/input.txt", "r") as in_file:
        cost_data:Dict[Tuple[int,int],int] = dict()
        ...
        for line in in_file:
            ...
            for c in line:
                ...
                x += 1
            y += 1
        for x_scale in range(0,5):
            for y_scale in range(0,5):
                if x_scale == 0 and y_scale == 0:
                    continue
                for x_i in range(x):
                    for y_i in range(y):
                        new_value = cost_data[(x_i,y_i)] + (x_scale + y_scale)
                        if new_value > 9:
                            new_value-=9
                        cost_data[(x_i+(x_scale*x),y_i+(y_scale*y))] = new_value
        x *= 5
        y *= 5
