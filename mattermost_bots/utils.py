...
def rename_bot(base_url, old_bot_name, old_bot_id, new_bot_name, headers):
    ...
    resp = requests.put(
        base_url + "api/v4/bots/" + old_bot_id,
        headers=headers,
...
def update_bot_icon(base_url, bot_name, bot_id, new_image_file, headers):
    ...
    resp = requests.post(
        base_url + "api/v4/bots/" + bot_id + "/" + "icon",
        headers=new_headers,
        files=files,
    )
    ...
    if resp.status_code < 200 or resp.status_code > 299:
        ...
        print(
            "Updating icon currently doesn't work even though server responded with 200/OK:"
        )
...
def return_computed_delay(target_time):
    ...
    print(
        f"I need to sleep until {target_time}, that's {delay}, or {delay.seconds} seconds"
    )
...
def create_dm_channel(base_url, bot_id, user_id, header):
    ...
    channel_info = requests.post(
        base_url + "api/v4/channels/direct",
        headers=header,
