    ...
    if any(channels):
        for _, channel in channels.iterrows():
            ...
            while True:
                ...
                print(
                    f"Retrieving info for users in channel {channel_name}, page {page} ({results_per_page} entries per page)"
                )
                resp = requests.get(
                    users_url + f"?page={page}&in_channel={channel_id}", headers=headers
                )
                new_dict = json.loads(resp.text)
                all_users += new_dict
        ...
        while True:
            ...
            print(
                f"Retrieving info for users, page {page} ({results_per_page} entries per page)"
            )
...
def process_reactions(args, reactions, users_url, headers, all_users):
    ...
    for reaction in reactions:
        if args.emoji == "*":
            all_users.loc[
                all_users["id"] == reaction["user_id"], "Emoji Response(s)"
            ] += (reaction["emoji_name"] + "|")
        elif args.emoji == reaction["emoji_name"]:
            all_users.loc[
                all_users["id"] == reaction["user_id"], f"Responded with {args.emoji}?"
            ] = "Yes"
    return all_users
...
def send_dm(base_url, channel_id, header, message):
    ...
    dm_info = requests.post(
        base_url + "api/v4/posts",
        headers=header,
...
# ==============================================================================
def send_dm_to_all_in_df(
    user_id, user_name, base_url, bot_id, header, message, live_run, bot_name
):
    """
...
def search_hashtags(args, url, headers, team_id, channels):
    ...
    for channel_index, channel_id in enumerate(channels["id"].values):
        ...
        for page in range(6):
            print(
                f"Getting Page {page} of {channels.iloc[channel_index]['name']} posts"
            )
            # headers['page'] = page
            resp = requests.get(
                get_posts_url + f"?page={page}", headers=headers, data=json.dumps(data)
            )
            if resp.status_code < 200 or resp.status_code > 299:
                ...
                desired_messages = messages[
                    messages["hashtags"].str.contains(args.keyword)
                ]
                print(f"Found {len(desired_messages)} results")
                if results.empty:
                    results = desired_messages[
                        ["id", "create_at", "message", "hashtags"]
                    ]
                else:
                    results = results.append(
                        desired_messages[["id", "create_at", "message", "hashtags"]]
                    )
...
# ==============================================================================
def send_accountability_message(
    args,
    url,
    headers,
    all_users,
    ...
    if args.emoji == "*":
        ...
        print(
            f"The following {len(posters)} users HAVE posted at least 1 emoji on post {args.post_id}"
        )
        pprint.pprint(posters[["username", "first_name", "last_name"]])
    else:
        posters = all_users[all_users[f"Responded with {args.emoji}?"] == "Yes"]
        print(
            f"The following {len(posters)} users HAVE posted the '{args.emoji}' emoji on post {args.post_id}"
        )
        pprint.pprint(posters[["username", "first_name", "last_name"]])
    ...
    if args.emoji == "*":
        ...
        print(
            f"The following {len(non_posters)} users have NOT posted any emoji on post {args.post_id}"
        )
        ...
        print(
            f"The following {len(non_posters)} users have NOT posted the '{args.emoji}' emoji on post {args.post_id}"
        )
        pprint.pprint(non_posters[["username", "first_name", "last_name"]])

    # Send provided DM
    for recipients, message in [
        [posters, message_to_responders],
        [non_posters, message_to_non_responders],
    ]:
        ...
        if args.live_run:
            ...
            message_list = [
                message + f" (Post: {url}{team_name}/pl/{args.post_id})"
            ] * len(recipients)
...
def main(parser):
    ...
    if resp.status_code >= 200 and resp.status_code <= 399:
        ...
        all_users = all_users.merge(
            status[["id", "status", "manual"]], on="id", how="left"
        )
    pprint.pprint(all_users)
    ...
    if args.post_id:
        all_users = send_accountability_message(
            args,
            url,
            headers,
            all_users,
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--emoji", "-e", type=str, default="*", help="Name of emoji ('*' for any)"
    )
