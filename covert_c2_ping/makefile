...
$(64client): covert_c2_ping_client/src/*
	RUSTFLAGS='-C link-arg=-s' cargo build -Z build-std=std,panic_abort -Z build-std-features=panic_immediate_abort --target x86_64-pc-windows-gnu --release -p covert_c2_ping_client

$(server): covert_c2_ping_server/src/* covert_c2_ping_server/src/workers/*
	cargo build --target x86_64-unknown-linux-musl --release -p covert_c2_ping_server
